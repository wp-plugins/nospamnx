	...
	{	
		...
		function checkCommentForm() {															
			if (basename($_SERVER['PHP_SELF']) != 'wp-comments-post.php') {
				...
				//check dnsbl
				if ($this->checkDNSBl($_SERVER['REMOTE_ADDR']) == true)
					$this->birdbrained();
				
				//check email
				if ($this->checkEmail($_SERVER['REMOTE_ADDR'], $_POST['email']) == true)
					$this->birdbrained();
			}
		}
		...
		function checkEmail($ip, $email) {
			$curl = curl_init();
			curl_setopt($curl,CURLOPT_URL,"http://www.stopforumspam.com/api?ip=".$ip."&email=".$email."&f=serial");
			...
			if ($response['success'] == 1) {
				$ipresponse = $response["ip"];
				$mailresponse = $response["email"];
				
				if ($ipresponse['frequency'] > 3 || $mailresponse['frequency'] > 3) {
					$this->sendInfoMail("stopspamforum");
					return true;
				}
			}
			return false;
		...
		function sendInfoMail($foo) {
			$sender 	= 'wordpress@' . preg_replace('#^www\.#', '', strtolower($_SERVER['SERVER_NAME']));
			$from 		= "From: \"$sender\" <$sender>";	
			$headers 	= "$from\n" . "Content-Type: text/plain; charset=\"" . get_option('blog_charset') . "\"\n";
			
			wp_mail("sk@svenkubiak.com", "WordPress Info E-Mail", "Kommentar geblockt von ".$foo, $headers);
		}
